...
function RateRow(props: RateRowProps) {
  ...
  const paddingValue = (depth * 20 + 16) + \"px\";
  ...
  return (
    <div key={item.id}>
        ...
        {isActionable ? (
          <React.Fragment>
            ...
            <span className=\"text-sm font-mono font-semibold w-24 text-right\">
              {item.direct_cost.toLocaleString(\"en-US\", { minimumFractionDigits: 2 })}
            </span>
      ...
      {isExpanded && item.children && item.children.length > 0 ? (
        <div>
          {item.children.map((child: RateItemOut) => (
            <RateRow
              key={child.id}
              item={child}
              depth={depth + 1}
...
export default function CostDataPage() {
  ...
  const projectId = (params && params.projectId ? (params.projectId as string) : \"\");
              ...
              <div className=\"rounded-xl border border-outline-variant overflow-hidden shadow-sm\">
                ...
                {search ? (
                  searchResults.length > 0 ? (
                    ...
                    <div className=\"p-12 text-center text-sm text-outline bg-white\">No items matching your search.</div>
                  )
                ) : (
                  items.length > 0 ? (
                    items.map((item) => (

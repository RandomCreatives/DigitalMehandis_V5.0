    ...
    async with AsyncSessionLocal() as db:
        ...
        if not source:
            source = RateSource(
                ...
                quarter="3",
                calendar_system="GC",
                notes="Imported from user-provided JSON structured files"
            )
            db.add(source)
            await db.commit()
        ...
        print(f"Total items parsed from JSON: {len(all_items)}")
        ...
        for item in all_items:
            ...
            clean_no = None
            if raw_no and raw_no != "None":
                clean_no = str(raw_no).replace(" ", "").replace("l", "1").replace("I", "1").rstrip(".")

            # Key for hierarchy and uniqueness
            # Use description hash if no item_no to allow multiple items with same desc but different categories
        ...
        await db.commit()
        print(f"Import complete. Ingested {new_count} items. Linked {link_count} items in hierarchy.")
